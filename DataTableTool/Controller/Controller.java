package DataTableTool.Controller;
...
public class Controller {
    ...
    public ArrayList<String> get(JSONObject jsonObject, JSONObject codeset, String type) {
        try {
            ...
            for (int i = 0; i < temparr.size(); i++) {
                tempstr = MatchCode(codeset, temparr.get(i), type);
            ...
            String tempstr;
            tempstr = MatchCode(codeset, temp, type);
            temparr.add(tempstr);
            return temparr;
        }
    }
    ...
    public LinkedHashMap<String, ArrayList<Node<JSONObject>>> getTree(JSONObject jsonObject) {
        ...
        LinkedHashMap<String,ArrayList<Node<JSONObject>>> MemberMap = new LinkedHashMap<>();
        ...
        for (JSONObject dimension : dimensions){
            ...
            Node<JSONObject> root = new Node<>(null);
            ArrayList<Node<JSONObject>> roots = new ArrayList<>();
            for (JSONObject temp: members){
                if(temp.get("parentMemberId")==null){
                    Node<JSONObject> newNode = new Node<>(temp);
                    root = new Node<>(temp);
                    roots.add(root);
    ...
    public ArrayList<String> convert(ArrayList<Node<JSONObject>> arr) {
        ...
        for (Node<JSONObject> json : arr){
            temparrlist.add((String) json.getData().get("memberName"+currLanguage));
        }
        return temparrlist;
    }
    ...
    public JSONObject sendPostRequest(String passedURL, int passedId) {
        ...
        try {
            ...
            if (!tempstatus.equals("FAILED")) {
                ...
                return (JSONObject) response_JSON.get("object");
            }
    ...
    public String MatchCode(JSONObject jsonSets, String code, String type) {
        ...
        for (Object o : needed_CodeSet) {
            JSONObject temp = (JSONObject) o;
            try {
                int subjectCode = Integer.parseInt((String) temp.get(type + "Code"));
                if (subjectCode == Integer.parseInt(code)) {
                    return (String) (temp.get(type + currLanguage));
                }
            } catch (ClassCastException e) {
